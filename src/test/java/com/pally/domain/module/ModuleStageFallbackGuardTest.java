...
class ModuleStageFallbackGuardTest {
    ...
    private List<Supplier<List<ModuleContentItem>>> allStageGenerators() {
        ...
        return List.of(
                () -> gen.generateMicroCards("m1", content, "P5", "Science", "FREE", "", "av-test", "en"),   // LEARN
                () -> gen.generateHotTakes("m1", content, "P5", "Science", "FREE", "", "av-test", "en"),      // TEST
                () -> gen.generateSpotMistake("m1", content, "P5", "Science", "", "av-test", "en"),           // TEST
                () -> gen.generateChallenges("m1", content, "P5", "Science", "FREE", "", "av-test", "en"));   // TEST
    }
    ...
    private void assertEveryStageSubstantive(String modelReply) throws Exception {
        ...
        // The teacher "Regenerate" path is now the SAME merged generators called with a
        ...
        for (ModuleContentItem item : gen.generateSpotMistake("m1", content, "P5", "Science", guidance, "av-test", "en")) {
            assertSubstantive(item);
        }
        for (ModuleContentItem item : gen.generateMicroCards("m1", content, "P5", "Science", "FREE", guidance, "av-test", "en")) {
            assertSubstantive(item);
        }
        for (ModuleContentItem item : gen.generateHotTakes("m1", content, "P5", "Science", "FREE", guidance, "av-test", "en")) {
            assertSubstantive(item);
        }
        for (ModuleContentItem item : gen.generateChallenges("m1", content, "P5", "Science", "FREE", guidance, "av-test", "en")) {
            assertSubstantive(item);
        }
    }
